...
$choice = Read-Host "Select a model to install (1-8)"
...
if ($choice -eq "7") {
    Write-Host ""
    Write-Host "⚠️ You've chosen to install all models. This will download approximately 17GB of data." -ForegroundColor Yellow
...
foreach ($model in $modelsToPull) {
    Write-Host ""
    Write-Host "📥 Pulling $($model.DisplayName)... (approximately $($model.Size))" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "📋 Current models in Ollama:" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Example API call (PowerShell):" -ForegroundColor Yellow
Write-Host '$body = @{
    model = "llama2"
...
$response.response' -ForegroundColor Gray
